...
export default function CreateUser() {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      newUser?.userId &&
      newUser.officialUnitId &&
      [USER_ROLES.DATA_CONTRIBUTOR, USER_ROLES.INSTITUTE_DATA_CONTRIBUTOR].includes(newUser.role) &&
      ['Super Admin', 'MInT Admin', 'Regional Admin', 'Federal Admin', 'Institutional Admin'].includes(user?.role)
    ) {
      const unit = getUnitById(newUser.officialUnitId);
      notifyContributorUserAssignedToUnit(newUser.userId, unit?.officialUnitName || 'your assigned unit');
    }
